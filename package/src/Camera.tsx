...
export class Camera extends React.PureComponent<CameraProps> {
  ...
  private calculateBitRate(bitRate: 'low' | 'normal' | 'high', codec: 'h264' | 'h265' = 'h264'): number {
    ...
    if (format == null) throw new CameraRuntimeError('parameter/invalid-combination', `A videoBitRate of '${bitRate}' can only be used in combination with a 'format'!`)
  ...
  public startRecording(options: RecordVideoOptions): void {
    ...
    if (typeof videoBitRate === "string") {
      passThroughOptions.videoBitRate = this.calculateBitRate(videoBitRate, options.videoCodec)
    }
