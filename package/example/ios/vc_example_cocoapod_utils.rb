...
def does_example_have_worklets_enabled(installer)
  ...
  if defined?($EnableFrameProcessors)
    enableFrameProcessors = $EnableFrameProcessors
  end
  has_worklets = ReactNativePodsUtils.has_pod(installer, "react-native-worklets-core") && enableFrameProcessors
end
