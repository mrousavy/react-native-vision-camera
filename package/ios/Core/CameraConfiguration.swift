...
final class CameraConfiguration {
    ...
    struct Difference {
        ...
        var isDeviceConfigurationDirty: Bool {
            return isSessionConfigurationDirty || formatChanged || sidePropsChanged || zoomChanged || exposureChanged || temperatureChanged || tintChanged
        }
        ...
        init(between left: CameraConfiguration?, and right: CameraConfiguration) {
            ...
            temperatureChanged = inputChanged || left?.temperature != right.temperature
            
            tintChanged = inputChanged || left?.tint != right.tint
