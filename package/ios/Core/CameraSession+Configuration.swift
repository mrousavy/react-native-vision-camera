...
extension CameraSession {
    ...
    func configureTemperatureAndTint(configuration: CameraConfiguration, device: AVCaptureDevice){
        guard let temperature = configuration.temperature else {
            return
        }
        
        guard let tint = configuration.tint else {
            return
        }
        
        
        var gains = device.deviceWhiteBalanceGains(for: AVCaptureDevice.WhiteBalanceTemperatureAndTintValues(temperature: temperature, tint: tint))
