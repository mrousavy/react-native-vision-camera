...
fun CameraCharacteristics.getPreviewSizeFromAspectRatio(aspectRatio: Double): Size {
  ...
  val outputSizes = config.getOutputSizes(SurfaceHolder::class.java).sortedByDescending { it.width * it.height }.sortedBy { abs(aspectRatio - (it.bigger.toDouble() / it.smaller)) }
