 ...
 * in [ImageFormat.DEPTH16].
 */
@SuppressLint("RestrictedApi")
class DepthImageReaderProxy : ImageReaderProxy {
  private val imageReaderThread = HandlerThread("DepthImageReaderProxy").apply { start() }
  private val imageReaderHandler = Handler(imageReaderThread.looper)
  private val imageReader: ImageReader

  constructor(width: Int, height: Int, maxImages: Int, usage: Long) {
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
      imageReader = ImageReader.newInstance(width, height, ImageFormat.DEPTH16, maxImages, usage)
    } else {
      imageReader = ImageReader.newInstance(width, height, ImageFormat.DEPTH16, maxImages)
    }
  }
