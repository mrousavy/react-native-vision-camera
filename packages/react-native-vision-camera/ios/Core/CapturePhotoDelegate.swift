...
class CapturePhotoDelegate: NSObject, AVCapturePhotoCaptureDelegate {
  ...
  private let onCaptured: (AVCapturePhoto, MediaSampleMetadata) -> Void
  private let onError: (any Error) -> Void
  private let callbacks: CapturePhotoCallbacks
  private var didResolve = false

  init(onCaptured: @escaping (AVCapturePhoto, MediaSampleMetadata) -> Void,
  ...
  func photoOutput(_ output: AVCapturePhotoOutput, didFinishProcessingPhoto photo: AVCapturePhoto, error: (any Error)?) {
    if let error {
      ...
      let metadata = ConnectionMetadata(output: output)
