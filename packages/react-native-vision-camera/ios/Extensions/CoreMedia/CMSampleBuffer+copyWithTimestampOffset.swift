...
extension CMSampleBuffer {
  private func getTimingInfos() throws -> [CMSampleTimingInfo] {
    ...
    let getCountStatus = CMSampleBufferGetSampleTimingInfoArray(
      self,
      entryCount: 0,
      arrayToFill: nil,
    ...
    let emptyTimingInfo = CMSampleTimingInfo(
      duration: .invalid,
      presentationTimeStamp: .invalid,
      decodeTimeStamp: .invalid)
    var infos = [CMSampleTimingInfo](repeating: emptyTimingInfo, count: count)
    let getArrayStatus = CMSampleBufferGetSampleTimingInfoArray(
      self,
      entryCount: count,
      arrayToFill: &infos,
  ...
  func copyWithTimestampOffset(_ offset: CMTime) throws -> CMSampleBuffer {
    ...
    let copyResult = CMSampleBufferCreateCopyWithNewTiming(
      allocator: nil,
      sampleBuffer: self,
