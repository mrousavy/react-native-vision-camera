///
/// TargetPixelFormat.swift
/// This file was generated by nitrogen. DO NOT MODIFY THIS FILE.
/// https://github.com/mrousavy/nitro
/// Copyright © 2025 Marc Rousavy @ Margelo
///

/**
 * Represents the JS union `TargetPixelFormat`, backed by a C++ enum.
 */
public typealias TargetPixelFormat = margelo.nitro.camera.TargetPixelFormat

public extension TargetPixelFormat {
  /**
   * Get a TargetPixelFormat for the given String value, or
   * return `nil` if the given value was invalid/unknown.
   */
  init?(fromString string: String) {
    switch string {
      case "native":
        self = .native
      case "yuv-420-8-bit-video":
        self = .yuv4208BitVideo
      case "yuv-420-8-bit-full":
        self = .yuv4208BitFull
      case "yuv-420-10-bit-video":
        self = .yuv42010BitVideo
      case "yuv-420-10-bit-full":
        self = .yuv42010BitFull
      case "rgb-bgra-32-bit":
        self = .rgbBgra32Bit
      case "depth-32-bit":
        self = .depth32Bit
      case "depth-16-bit":
        self = .depth16Bit
      case "disparity-16-bit":
        self = .disparity16Bit
      case "disparity-32-bit":
        self = .disparity32Bit
      default:
        return nil
    }
  }

  /**
   * Get the String value this TargetPixelFormat represents.
   */
  var stringValue: String {
    switch self {
      case .native:
        return "native"
      case .yuv4208BitVideo:
        return "yuv-420-8-bit-video"
      case .yuv4208BitFull:
        return "yuv-420-8-bit-full"
      case .yuv42010BitVideo:
        return "yuv-420-10-bit-video"
      case .yuv42010BitFull:
        return "yuv-420-10-bit-full"
      case .rgbBgra32Bit:
        return "rgb-bgra-32-bit"
      case .depth32Bit:
        return "depth-32-bit"
      case .depth16Bit:
        return "depth-16-bit"
      case .disparity16Bit:
        return "disparity-16-bit"
      case .disparity32Bit:
        return "disparity-32-bit"
    }
  }
}
