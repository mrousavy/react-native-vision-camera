...
:::info
This needs to be handled manually, see [`Frame.bufferOrientation`](/docs/api/interfaces/Frame#bufferorientation).
For example, in MLKit just pass the `Frame`'s `bufferOrientation` to the `detect(...)` method.
:::
...
The Frame's [`bufferOrientation`](/docs/api/interfaces/Frame#bufferorientation) represents it's orientation relative to the current target orientation.

For example, if the phone is held in `portrait` mode and the Frame's `bufferOrientation` is `landscape-right`, it is 90° rotated and needs to be counter-rotated by -90° to appear "up-right".
...
public override func callback(_ frame: Frame, withArguments _: [AnyHashable: Any]?) -> Any? {
  ...
  // highlight-next-line
  mlImage.orientation = frame.bufferOrientation
  // ...
```
