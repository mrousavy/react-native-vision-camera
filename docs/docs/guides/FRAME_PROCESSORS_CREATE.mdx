...
iOS Frame Processor Plugins can be written in Objective-C or Swift.
...
3. Inside the `@implementation`, implement the plugin using a C-style function with the following signature:
...
:::info
The JS function name will be equal to the Objective-C function name you choose (with a `__` prefix). Make sure it is unique across other Frame Processors, and doesn't shadow a JS runtime variable.
:::
...
4. Start implementing your custom Frame Processor's code! 🚀
...
4. Add the following code to the Objective-C source file:
...
6. Start implementing your custom Frame Processor's code! 🚀
