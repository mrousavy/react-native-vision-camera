...
```java
@Override
public Object callback(Image image, Object[] params) {
  return "cat";
}
```
...
You can also manipulate the buffer and return it (or a copy of it) by returning a [`Frame`][2]/[`Image`][3] instance:
...
You can run any native code you want in a Frame Processor Plugin. Just like in the native iOS and Android Camera APIs, you will receive a frame (`CMSampleBuffer` on iOS, `Image` on Android) which you can use however you want. In other words; **everything is possible**.
...
[3]: https://developer.android.com/reference/android/media/Image
