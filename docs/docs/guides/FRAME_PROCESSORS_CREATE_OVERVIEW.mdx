...
Return values will automatically be converted to JS values, assuming they are representable in the ["Types" table](#types). So the following native Frame Processor Plugin:
...
public static func callback(_ frame: Frame!, withArgs args: [Any]!) -> Any! {
  ...
  return Frame(buffer: resizedBuffer, orientation: frame.orientation)
}
```
...
@objc
public static func callback(_ frame: Frame!, withArgs args: [Any]!) -> Any! {
  guard let serverURL = args[0] as? String else { return nil }
  ...
  if let bufferCopy = bufferCopy {
    DispatchQueue.global(qos: .userInitiated).async {
      WebRTC.upload(buffer: bufferCopy, toServer:serverURL)
    }
  }
...
public Object callback(@NotNull ImageProxy image, @NotNull Object[] params) {
  ...
  ImageProxy imageCopy = // ...

  uploaderQueue.async(new Runnable() {
    public void run() {
