...
- `enableDepthData`: Enables the depth camera configuration if `depthDataFormat` is specified (iOS only)
- `depthDataFormat`: Specifies the depth data format to use (can be depth maps or disparity maps, iOS only)
