...
There are formats specifically designed for high-resolution photo capture (but lower FPS), or formats that are designed for slow-motion video capture which have frame-rates up to 240 FPS (but lower resolution).
...
* Each camera device is built differently, e.g. _Telephoto devices_ often don't provide frame-rates as high as _Wide-Angle devices_. (see ["Camera Devices"](devices))
...
* Different formats provide different field-of-views (FOV), maximum zoom factors, resolutions, frame rate ranges, and systems to assist with capture (auto-focus systems, video stabilization systems, ...)
