...
title: Creating Frame Processor Plugins (iOS)
...
The following return- and parameter-types are supported:
* `NSString` (`string` in JS)
* `NSNumber` (`number` in JS)
* `nil` (`undefined` or `null` in JS)
* `NSDictionary` (`{}` in JS)
* `NSArray` (`[]` in JS)
* `RCTResponseSenderBlock` (`(any...) => void` in JS)
...
export function scanQRCodes(frame: Frame): string[] {
  ...
  // or with args:
  return __scanQRCodes(frame, "hello!", "parameter 2", 42, true, { enableMagic: true });
}
```
