...
By default, VisionCamera will use the device's built-in microphone for audio recording. If you want to use a different microphone, you can get a list of available audio devices using the `useCameraDevices()` hook:

```tsx
const { audio: audioDevices } = useCameraDevices()
```

Then, you can pass the desired audio device to the `startRecording` function:

```ts
const bluetoothMic = audioDevices.find(d => d.type === 'bluetooth-microphone')

camera.current.startRecording({
  ...props,
  audioDevice: bluetoothMic,
})
```

VisionCamera will then use the selected microphone for the recording.
