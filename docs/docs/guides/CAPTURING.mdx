...
:::note
The `takeSnapshot` function also works with `photo={false}`. For this reason VisionCamera will automatically fall-back to snapshot capture if you are trying to use more use-cases than the Camera natively supports. (see ["The `hardwareLevel` prop"](/docs/api/interfaces/CameraDevice#hardwarelevel))
:::
