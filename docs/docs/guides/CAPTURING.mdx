...
:::note
The `takeSnapshot` function also works with `photo={false}`. For this reason, devices that do not support as many use-cases as you need, (see ["Use-cases"](/docs/guides/devices/#use-cases)) can use other use-cases (such as `video={true}` or `frameProcessor={...}`) and **fall back to snapshot capture** for photos.
:::
