...
* [munishbp/**react-native-mediapipe-pose-plugin**](https://github.com/munishbp/react-native-mediapipe-pose-plugin): A frame processor plugin for real-time on-device pose estimation using Google MediaPipe Pose Landmarker, with native Swift (iOS) and Kotlin (Android) implementations, GPU acceleration, and 33 3D landmarks per frame.
