...
1. On Android, Skia Frame Processors currently only work with the `rgb` [pixel format](/docs/guides/pixel-formats). On iOS, you can use both `yuv` and `rgb`.
2. Skia Frame Processors are currently preview-only. Any content drawn to the `Frame` will not be visible in captured photos, snapshots or videos.
