...
A [`Barcode`](/api/react-native-vision-camera-barcode-scanner/hybrid-objects/Barcode)'s precise location in MLKit's Barcode coordinate system for the input image can be accessed via [`boundingBox`](/api/react-native-vision-camera-barcode-scanner/hybrid-objects/Barcode#boundingbox) (a rectangle covering the barcode), or [`cornerPoints`](/api/react-native-vision-camera-barcode-scanner/hybrid-objects/Barcode#cornerpoints) (points for each corner):

```ts
const barcode = ...
console.log(barcode.boundingBox) // { left: 120, right: 540, top: 80, bottom: 260 }
console.log(barcode.cornerPoints) // [{ x: 120, y: 80 }, ...]
```

#### Converting Barcode coordinates to View coordinates

To convert Barcode coordinates to Preview View coordinates, first convert them to Camera coordinates using [`Barcode.convertBarcodePointToCameraPoint(...)`](/api/react-native-vision-camera-barcode-scanner/hybrid-objects/Barcode#convertbarcodepointtocamerapoint), and then use [`PreviewView.convertCameraPointToViewPoint(...)`](/api/react-native-vision-camera/interfaces/PreviewViewMethods#convertcamerapointtoviewpoint):
