...
### Converting between Frames and Images

The [`HybridFrameConverter`](/api/react-native-vision-camera/variables/HybridFrameConverter) converts a [`Frame`](/api/react-native-vision-camera/hybrid-objects/Frame) to an [`Image`](https://github.com/mrousavy/react-native-nitro-image) (from [react-native-nitro-image](https://github.com/mrousavy/react-native-nitro-image)), and an [`Image`](https://github.com/mrousavy/react-native-nitro-image) back to a [`Frame`](/api/react-native-vision-camera/hybrid-objects/Frame):
...
// ...or create a camera-like Frame from any Image:
const frame = HybridFrameConverter.convertImageToFrame(image, 'up', false)
```

[`convertImageToFrame(...)`](/api/react-native-vision-camera/hybrid-objects/FrameConverter#convertimagetoframe) creates a camera-like RGB [`Frame`](/api/react-native-vision-camera/hybrid-objects/Frame) (BGRA on iOS, RGBA on Android - a lossless conversion) as if the Camera had streamed it in the given [`orientation`](/api/react-native-vision-camera/hybrid-objects/Frame#orientation) and [`isMirrored`](/api/react-native-vision-camera/hybrid-objects/Frame#ismirrored) mode - the pixel data is physically rotated and mirrored accordingly.
This is useful for running Frame-based APIs (like a Barcode Scanner or the [Resizer](/docs/resizer)) on still images, or for testing Frame-processing code without a physical Camera.
