...
To allow better interop with third party libraries without adding native dependencies, a [`Frame`](/api/react-native-vision-camera/hybrid-objects/Frame) exposes it's underlying [`NativeBuffer`](/api/react-native-vision-camera/interfaces/NativeBuffer).
