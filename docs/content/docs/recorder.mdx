...
By default, recordings on iOS are written to a QuickTime (`.mov`) container. If the consumer expects MPEG-4, set `fileType: 'mp4'` on the [`VideoOutputOptions`](/api/react-native-vision-camera/type-aliases/VideoOutputOptions) when creating the output:
...
Android always writes `.mp4` and ignores this option.
