...
To automatically stop a recording when a certain duration or file-size has been reached, use [`maxDuration`](/api/react-native-vision-camera/interfaces/VideoOutputOptions#maxduration) or [`maxFileSize`](/api/react-native-vision-camera/interfaces/VideoOutputOptions#maxfilesize):
