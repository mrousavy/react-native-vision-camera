...
By default, recordings on iOS are written to a QuickTime (`.mov`) container. If the consumer expects MPEG-4, set `fileType: 'mp4'` on the [`VideoOutputOptions`](/api/react-native-vision-camera/interfaces/VideoOutputOptions) when creating the output:
