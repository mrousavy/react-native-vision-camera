...
<div align="center">
  <img className="doc-image" src="/img/action_mode_demo.gif" />
</div>
...
```ts
const format = getCameraFormat(device, [{ videoStabilizationMode: 'cinematic-extended' }])
```
...
- For [Frame Processors](/docs/guides/realtime-frame-processing): Frames will arrive at a later point in time. If a Frame just arrived with `cinematic-extended` video stabilization, it might already be 3 seconds old.
- For [Skia Frame Processors](/docs/guides/realtime-frame-processing/skia): Since Skia Frame Processors have to process the Frames first before drawing them onto the Preview, this now means that the Preview is also delayed by the latency of the video stabilization algorithm. If you use Skia Frame Processors, it is recommended to use fast software-based-, or no video stabilization at all.
