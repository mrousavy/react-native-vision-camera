...
if (CameraModule == null) {
  ...
  if (ExpoConstants != null) {
    if (ExpoConstants.appOwnership === 'expo') {
      throw new CameraRuntimeError(
        'system/camera-module-not-found',
        'react-native-vision-camera is not supported in Expo Go! Use EAS/expo prebuild instead (`npx expo prebuild`). For more info, see https://docs.expo.dev/workflow/prebuild/',
      );
    } else {
      message += `\n* Make sure you rebuilt the app using \`npx expo prebuild\` and \`npx expo run:${Platform.OS}\`. For more info, see https://docs.expo.dev/workflow/prebuild/`;
    }
  }
